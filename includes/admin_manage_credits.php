            ...
            if( $update != "" ) 
            {
                ...
                if( $user_id != "" && $user_point != "" ) 
                {
                    ...
                    for( $i = 0; $i < $count; $i++ ) 
                    {
                        ...
                        {
                            connectgamecpdb();
                            $select_user = "" . "SELECT userId,cashBalance FROM BillCruxRF.dbo.tblUserInfo WHERE userId = '" . $userid . "'";
                            $select_result = mssql_query($select_user);
                            $user = mssql_fetch_array($select_result);
                            if( $user["cashBalance"] != $userpoint ) 
                            {
                                $update_points = "UPDATE BillCruxRF.dbo.tblUserInfo SET cashBalance = '" . $user_point[$i] . "' WHERE userId = '" . $user_id[$i] . "'";
